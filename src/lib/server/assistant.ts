...
import { assistantDayHoursSchema, groundedDayHours, dateConstraintEvidence, openEndedProject } from "../assistant-day-hours";
import { defaultWorkPriority } from "../work";
...
export const assistantActionSchema = z.object({
  type: z.enum(["create", "fit", "update", "schedule", "move", "progress", "status", "client_update", "block", "set_day_hours"]),
  ...
  dayHours: assistantDayHoursSchema,
  dateConstraints: z.object({ earliestStart: nullableText, allowedDates: z.array(z.string()).max(366) }).strict().nullable(),
  allowedDates: z.array(z.string()).max(90),
...
function groundedDate(date: string, source: string, today: string, defaultToday: boolean) {
  ...
  if (/\byesterday\b/i.test(source) && date === addDays(today, -1)) return true;
...
function explicitPriorityRequested(priority: Context["priorities"][number], source: string) {
  const labels = [...new Set([clean(priority.id), clean(priority.label)])].filter(Boolean);
  return source.split(/[.!?\n;,]+/).some(statement => {
    const normalized = clean(statement);
    return labels.some(label => {
      // A task/client description may contain "normal", "high", or "low".
      // Only priority wording, an explicit assignment, or a short answer wins
      // over the category default.
      if (new RegExp(`\\b(?:not|never|don t|do not)\\s+(?:\\w+\\s+){0,4}${label}\\b`).test(normalized)) return false;
      return new RegExp(`^(?:please\\s+)?${label}(?:\\s+please)?$`).test(normalized)
        || new RegExp(`\\b${label}\\s+priority\\b|\\bpriority\\s+(?:(?:is|of|as|to|should be)\\s+)?${label}\\b`).test(normalized)
        || new RegExp(`\\b(?:set|change|make|mark|keep)\\s+(?:(?:it|this|that|(?:the|this|that)\\s+(?:work|task|project))\\s+)?(?:to|as)\\s+${label}\\b`).test(normalized)
        || new RegExp(`\\b(?:use|choose|select)\\s+${label}(?:\\s+please)?$`).test(normalized);
    });
  });
}
...
export function compileInterpretation(raw: unknown, text: string, state: Context, actor: Actor, now = new Date(), authorityText = text, clarificationReplies: string[] = [], dateSelection?: AssistantDateSelection | null): Interpretation {
  ...
  for (const action of output.actions) {
    ...
    const createEvidence = action.type === "create"
      ...
      : action.sourceQuote;
    if (action.dateConstraints) {
      for (const field of ["earliestStart", "allowedDates"] as const) {
        const values = field === "allowedDates" ? action.dateConstraints.allowedDates : action.dateConstraints.earliestStart ? [action.dateConstraints.earliestStart] : [];
        const evidence = dateConstraintEvidence(createEvidence, field);
        if (values.some(date => !validDate(date) || !evidence || !groundedDate(date, evidence, today, false)))
          return clarify("A booking day is flexible. Please explicitly state an earliest start or allowed work dates if you want a lasting date restriction.");
      }
      if (action.type !== "create" && !action.dateConstraints.earliestStart && !action.dateConstraints.allowedDates.length
        && !/\b(?:remove|clear)\b[^.!?\n]{0,50}\b(?:date restrictions|date constraints|earliest start|allowed (?:work )?dates)\b/i.test(createEvidence))
        return clarify("Please explicitly ask to clear the date restrictions before changing them.");
    }
    if (action.deadline && !projectDateFieldEvidence(createEvidence, "deadline")) action.deadline = null;
    const datedHours = groundedDayHours(action.dayHours, createEvidence, (date, quote) => groundedDate(date, quote, today, false), action.type !== "create");
    if (datedHours.error) return clarify(datedHours.error);
    if (datedHours.days?.some(day => dateSelection && !withinSelection(day.date, dateSelection)))
      return clarify("The daily hours fall outside the selected dates. Change the selection or use those dates.");
    if (action.type === "set_day_hours" && datedHours.days) {
      if (!item || !datedHours.days || !/\b(?:set|make|change|resize|reduce|shorten|remove|clear)\b/i.test(createEvidence))
        return clarify("State the final booked hours for each day of this existing project.");
      commands.push({ type: "set_day_hours", itemId: item.id, days: datedHours.days, ...permissionFlags(authorityText, actor, [item.title, item.id]) });
      continue;
    }
    ...
    const wantsFit = asksToFindTime(createEvidence, action.type !== "create") || action.type === "create"
      && createEvidence.split(/[.!?\n;,]+/).some(part => /\b(?:book|reserve)\b/i.test(part) && hasEffort(part) && asksToFindTime(part, true));
    ...
    const automaticFit = ["fit", "schedule", "update", "create"].includes(action.type) && wantsFit && !statedClockRanges(createEvidence).length && !legacyDailyAllocation && !datedHours.days;
    ...
    const daily = datedHours.days ? { plan: datedHours.days, total: datedHours.days.reduce((sum, day) => sum + day.minutes, 0) } : ["create", "update", "schedule", "fit", "set_day_hours"].includes(action.type)
    ...
    if (item && (action.type === "set_day_hours" || daily.plan && !automaticFit)) {
      if (!daily.plan || !/\b(?:set|make|change|resize|reduce|shorten|schedule|replace)\b/i.test(createEvidence))
        return clarify("State the final booked hours for each day of this existing project.");
      commands.push({ type: "set_day_hours", itemId: item.id, days: daily.plan, ...permissionFlags(authorityText, actor, [item.title, item.id]) });
      continue;
    }
    ...
    if (daily.plan && action.type === "create" && !unknownTotal) {
      const statedTotal = createEvidence.match(/\b(?:total(?:\s+(?:effort|estimate|hours))?\s*(?:of|is|:)?\s*)(\d+(?:\.\d+)?)\s*(hours?|minutes?)\b|\b(\d+(?:\.\d+)?)\s*(hours?|minutes?)\s+(?:in\s+)?total\b/i);
      if (statedTotal && Number(statedTotal[1] ?? statedTotal[3]) * (/hour/i.test(statedTotal[2] ?? statedTotal[4]) ? 60 : 1) !== daily.total)
        return clarify("The stated project total and the daily hours disagree. Please choose the daily amounts or correct the total.");
      action.estimatedMinutes = daily.total!;
    }
    ...
    if (automaticFit) {
      ...
      const startDate = action.allowedDates.length === 1 ? action.allowedDates[0] : dateSelection?.kind === "work_window" ? action.windowStart && matchesDate(action.windowStart, displayEvidence, false) ? action.windowStart : dateSelection.start : action.windowStart;
      const endDate = action.allowedDates.length === 1 ? action.allowedDates[0] : dateSelection?.kind === "work_window" ? action.windowEnd && matchesDate(action.windowEnd, displayEvidence, false) ? action.windowEnd : dateSelection.end : action.windowEnd;
    ...
    if (action.type === "create") {
      ...
      const windowStart = narrowedStart ?? dateSelection?.start ?? action.windowStart ?? monthSpan?.start ?? datedHours.days?.[0].date ?? workDates[0] ?? today;
      const bookingWindowEnd = narrowedEnd ?? dateSelection?.end ?? action.windowEnd ?? monthSpan?.end ?? datedHours.days?.at(-1)?.date ?? null;
      const windowEnd = openEndedProject(createEvidence) && !monthSpan && !timelineOnly ? null : bookingWindowEnd;
      if (windowEnd && windowEnd < windowStart) return clarify("The end of the work window must be on or after its start.");
      // A model-supplied fallback must not replace the category default. Keep
      // explicit choices editable while deriving unspecified priority here.
      const explicitPriority = priority && (explicitPriorityRequested(priority, createEvidence)
        || priority.id === "urgent" && urgent) ? priority : null;
      const categoryPriorityId = defaultWorkPriority(action.category, state.priorities);
      if (action.category === "it" && categoryPriorityId === "urgent" && !explicitPriority
        && /\b(?:not urgent|not an emergency|non[- ]urgent)\b/i.test(createEvidence))
        return clarify("IT work defaults to Urgent. Which other priority should I use for this work?");
      const id = randomUUID();
      const item: WorkItem = {
        ...
        priorityId: actor.role === "owner" ? explicitPriority?.id ?? categoryPriorityId : defaultWorkPriority("web", state.priorities),
        requestedPriorityId: actor.role === "requester" ? explicitPriority?.id ?? (action.category === "it" ? categoryPriorityId : null) : null, status: waiting ? "waiting" : "planned",
        ...
        forecastDate: null, completedAt: null, blockedReason: waiting ? (action.reason ?? (timelineOnly ? "Project timeline only; work sessions not yet scheduled" : action.estimatedMinutes === null ? "Awaiting estimate and scheduling details" : "Awaiting client input")) : null, minimumSessionMinutes: state.settings.slotMinutes,
        allowedDates: [], dateConstraints: action.dateConstraints ?? { earliestStart: null, allowedDates: [] },
        timelineMode: unknownTotal || monthSpan || timelineOnly || openEndedProject(createEvidence) ? "span" : "bookings", checklist: [], progressTotal: action.progressTotal, progressCompleted: 0, updateDate: action.updateDate,
        ...(daily.plan ? { dailyPlan: daily.plan } : {}),
      ...
      const bookingStart = action.allowedDates.length === 1 ? action.allowedDates[0] : windowStart;
      const bookingEnd = action.allowedDates.length === 1 ? action.allowedDates[0] : bookingWindowEnd ?? action.windowStart ?? action.deadline ?? windowStart;
      if (!smartFit && !datedHours.days && !waiting && !action.sessions.length && !flags.overrideProtected && !flags.overrideDeadline
        && !daily.plan?.some(day => !state.settings.weekdays.includes(dayOfWeek(day.date)))) {
        if (action.allowedDates.length > 1 && !dateSelection && !/\b(?:from|through|between|until)\b/i.test(createEvidence))
          return clarify("For separate workdays, state the hours for each day, or choose one continuous booking range.");
        ...
        if (amount.minutes !== undefined && (daily.plan || amount.minutes === item.remainingMinutes))
          smartFit = { startDate: bookingStart, endDate: bookingEnd, minutes: amount.minutes, distribution: daily.plan ? "per_day" : "total" };
      }
      ...
      for (const key of ["title", "description", "category", "webKind", "estimatedMinutes", "remainingMinutes", "windowStart", "windowEnd", "targetDate", "deadline", "progressTotal", "updateDate"] as const) {
        const value = action[key];
        if (value !== null) Object.assign(patch, { [key]: value });
      }
      if (priority) patch.priorityId = priority.id;
      if (action.dateConstraints) patch.dateConstraints = action.dateConstraints;
      ...
      if (daily.plan) commands.push({ type: "update", itemId: item.id, patch: { dailyPlan: daily.plan, windowStart: daily.plan[0].date, windowEnd: daily.plan.at(-1)!.date }, ...flags });
      ...
      if (item.status === "completed" || item.status === "cancelled" || session && session.status !== "planned")
        return clarify("Only planned work can move. Completed and cancelled sessions stay unchanged.");
      if (!action.sessions.length) {
        if (!/\b(?:move|reschedule|shift)\b/i.test(createEvidence) || /\b(?:said|wrote|quoted|forwarded|what if|maybe)\b|\b(?:do not|don't|never)\s+(?:move|reschedule|shift)\b/i.test(createEvidence))
          return clarify("Please directly request which planned work to move and its destination day.");
        const destination = [...createEvidence.matchAll(/\b(?:to|onto|into)\s+(.+)/gi)].at(-1);
        if (!destination || !action.windowStart || !groundedDate(action.windowStart, destination[1], today, false)
          || action.windowEnd && action.windowEnd !== action.windowStart || statedClockRanges(createEvidence).length
          || /\b\d{1,2}(?::\d{2})?\s*(?:am|pm)\b|\b(?:at|before|after)\s+(?:noon|midnight|\d)/i.test(createEvidence))
          return clarify("State one destination day, or give exact session times. I will keep the existing booked hours.");
        const sourceText = createEvidence.slice(0, destination.index);
        const hasSourceDay = /\b(?:today|yesterday|tomorrow|\d{4}-\d{2}-\d{2}|sunday|monday|tuesday|wednesday|thursday|friday|saturday|january|february|march|april|may|june|july|august|september|october|november|december)\b/i.test(sourceText);
        const candidates = state.sessions.filter(s => s.workItemId === item.id && s.status === "planned"
          && (!hasSourceDay || groundedDate(localDate(new Date(s.start), state.settings.timeZone), sourceText, today, false)));
        const sourceDays = new Set(candidates.map(s => localDate(new Date(s.start), state.settings.timeZone)));
        if (!candidates.length || sourceDays.size !== 1 || action.sessionId && !candidates.some(s => s.id === action.sessionId))
          return clarify("Which planned day should I move? Include its source date and destination; completed or cancelled sessions cannot move.");
        // Any requested amount must parse before a whole-booking move is safe.
        // Unclear quantities (such as "some hours") must not become "all hours".
        const amount = /\b(?:hours?|hrs?|minutes?|mins?)\b/i.test(createEvidence) ? statedFitMinutes(createEvidence) : {};
        const total = candidates.reduce((sum, s) => sum + (Date.parse(s.end) - Date.parse(s.start)) / 60_000, 0);
        if (amount.error || amount.minutes !== undefined && (amount.minutes > total || candidates.length > 1 && amount.minutes !== total))
          return clarify("Move the day's existing hours together, or name one clock-time booking for a partial transfer.");
        if (flags.overrideDeadline || candidates.length > 1 && flags.overrideProtected)
          return clarify("Select one specific booking and its exact new times to review this override.");
        commands.push(candidates.length > 1
          ? { type: "move_bookings", sessionIds: candidates.map(s => s.id), date: action.windowStart }
          : { type: "move_booking", sessionId: candidates[0].id, date: action.windowStart, ...(amount.minutes !== undefined ? { minutes: amount.minutes } : {}), ...flags });
      } else {
        if (!session || action.sessions.length !== 1) return clarify("Select the specific planned session and give its new start and end times.");
...
export function emptyAssistantAction(type: AssistantAction["type"], sourceQuote: string): AssistantAction {
  return { type, sourceQuote, clientName: null, itemReference: null, sessionId: null, title: null, description: null, category: null, webKind: null, priorityLabel: null,
    estimatedMinutes: null, remainingMinutes: null, windowStart: null, windowEnd: null, targetDate: null, deadline: null, dayHours: [], dateConstraints: null, allowedDates: [],
