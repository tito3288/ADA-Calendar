...
export function compileInterpretation(raw: unknown, text: string, state: Context, actor: Actor, now = new Date(), authorityText = text, clarificationReplies: string[] = []): Interpretation {
  ...
  for (const action of output.actions) {
    ...
    const waiting = action.type === "create" && actor.role === "owner" && waitingRequested && (action.estimatedMinutes === null || action.status === "waiting");
    // Month language can ground the display span of waiting work ONLY. It must
    // never authorize actual work dates, booked sessions, targets, or deadlines.
    const monthEvidence = waiting && !action.sessions.length ? projectMonthEvidence(createEvidence) : null;
    ...
    const displayEvidence = waiting ? createEvidence : action.sourceQuote;
