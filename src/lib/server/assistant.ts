...
export function compileInterpretation(raw: unknown, text: string, state: Context, actor: Actor, now = new Date(), authorityText = text, clarificationReplies: string[] = []): Interpretation {
  ...
  for (const action of output.actions) {
    ...
    if (action.type === "create") {
      ...
      const waitingRequested = /\b(?:unscheduled|no estimate|without an? estimate|unknown effort|waiting|on hold|blocked)\b/i.test(action.sourceQuote);
      const waiting = actor.role === "owner" && waitingRequested && (action.estimatedMinutes === null || action.status === "waiting");
      if (action.estimatedMinutes === null && !waiting) return clarify(actor.role === "owner"
      ...
      if (action.status === "waiting" && !waiting) return clarify("Should this work wait without reserving time, or should I schedule it? Please state that explicitly.");
