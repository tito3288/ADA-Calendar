-- Planned work remains movable after its scheduled time. Time passing is not
-- completed work. No stored bookings, estimates, settings, or events change.
-- Keep future placement, authorization, protections, and exact Undo validation.
...
returns bigint language plpgsql security definer set search_path='' as $$
declare member public.workspace_members; command jsonb; day jsonb; total numeric; session jsonb; minimum numeric;
begin
  ...
  if not exists(select 1 from public.work_events where workspace_id=member.workspace_id and operation_id=p_proposal->>'operationId') then
    -- Elapsed planned bookings remain editable. Recorded completion and
    -- cancellation remain history, including for direct authenticated RPCs.
...
returns bigint language plpgsql security definer set search_path='' as $$
begin
  -- The wrappers authorize and lock the workspace, preserve recorded history,
  -- and validate daily hours. The core checks exact latest-event Undo and
  -- idempotent retries before permitting restoration of an old planned time.
